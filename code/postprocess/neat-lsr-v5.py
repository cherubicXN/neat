...
def wireframe_recon(**kwargs):
    ...
    line_path = os.path.join(wireframe_dir,'{}-d={}-s={}-v5.npz'.format(kwargs['checkpoint'],kwargs['distance'],kwargs['score']))
    ...
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        # for s, lb, lines_gt in zip(split,split_label,split_lines):
            ...
            # temp = model.attraction_network.feature(points3d_)
            # is_ok = (temp.argmax(dim=-1) == out['ins'].argmax(dim=-1))
            # if is_ok.sum() == 0:
            #     continue

            # lines3d.append(lines3d_[is_ok])
            # lines2d.append(lines2d_[is_ok])
            # points3d.append(points3d_[is_ok])
            # lines_gt = lines_gt[:,:-1]
    ...
    scores_all = torch.cat(scores_all,dim=0).cpu().numpy()
    ...
    np.savez(line_path,lines3d=lines3d_all,scores=scores_all,cameras=cameras),#scores=scores_all,points3d_all=points3d_all)
