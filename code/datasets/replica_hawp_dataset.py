...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        image_dir = '{0}/results'.format(self.instance_dir)
        image_paths_tmp = sorted(utils.glob_imgs(image_dir))
        image_paths_tmp = sorted([img_p for img_p in image_paths_tmp if 'frame' in img_p])
        n_images_tmp = len(image_paths_tmp)
        self.cam_file = '{0}/traj.txt'.format(self.instance_dir)
        pose_all, intrinsics_all = self.load_poses_intrinsics(self.cam_file, n_images_tmp)
        ...
        for i in range(n_images_tmp):
            if i % sample_step == 0:
                self.pose_all.append(pose_all[i])
                self.intrinsics_all.append(intrinsics_all[i])
                image_paths.append(image_paths_tmp[i])
    ...
    def load_poses_intrinsics(self, path, n_images):
        pose_all = []
        intrinsics_all = []
        fx = 600.0
        fy = 600.0
    ...
    def compute_point_line_attraction(self, lines):
        ...
        lines_ = lines[:, :-1].cuda()
        lmap, labels_onehot, _ = _C.encodels(lines_, self.img_res[0], self.img_res[1], self.img_res[0], self.img_res[1],
                                             lines_.shape[0])
