...
def wireframe_recon(**kwargs):
    ...
    lines3d_all = []
    for pts in torch.split(vertices,50000):
        
        import pdb; pdb.set_trace()
    line_path = os.path.join(wireframe_dir,'{}-d={}-s={}-v6.npz'.format(kwargs['checkpoint'],kwargs['distance'],kwargs['score']))
    ...
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        # for s, lb, lines_gt in zip(split,split_label,split_lines):
            ...
            is_sim = normal_sim>0.75
            if is_sim.sum()>0:
                lines3d.append(lines3d_[is_sim])
                lines2d.append(lines2d_[is_sim])
