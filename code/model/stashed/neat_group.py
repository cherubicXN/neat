...
class AttractionFieldNetwork(nn.Module):
    def __init__(self,
        ...
        if multires>0:
            embed_fn, input_ch = get_embedder(multires, input_dims=d_in)
            self.embed_fn = embed_fn
            dims[0] = input_ch + feature_vector_size + 3
        self.num_layers = len(dims)
        for l in range(0, self.num_layers - 1):
            out_dim = dims[l+1]
            lin = nn.Linear(dims[l], out_dim)
    ...
    def forward(self, points, normals, feature_vectors):
        if self.embed_fn:
            x = torch.cat((self.embed_fn(points),normals,feature_vectors),dim=-1)
        else:
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        # self.attention(points_features,self.latents)
        ...
        l3d_feature = self.implicit_network(l3d)[:,1:]
        l3d_feature = self.mlp(l3d_feature.detach())
        # import pdb; pdb.set_trace()
        latents, latent_attn = self.attention(self.latents,l3d_feature,l3d_feature)
        lines3d_query = self.ffn(latents).reshape(-1,3)
        _, query_feats, query_grads = self.implicit_network.get_outputs(lines3d_query)
        latent_queries = self.attraction_network(lines3d_query.detach(),query_grads.detach(),query_feats.detach())
        lines2d_query = self.project2D(intrinsics[0,:3,:3], R, T, latent_queries)
        ...
        # lines3d = torch.stack((e1,e2),dim=1)
            lines2d = self.project2D(intrinsics[0,:3,:3], R, T, lines3d)
        latent_score, latent_label = latent_attn.max(dim=0)
        output['latent-labels'] = latent_label.detach()
        output['latent-score'] = latent_score.detach()
        output['latent-lines3d'] = latent_queries.detach()
        output['latent-lines2d'] = lines2d_query.detach()
        # output['lines3d-query'] = lines3d_query
        # output['lines2d-query'] = lines2d_query
