...
def project_point_to_line(line_segs, points):
    ...
    coords1d = coords1d.detach()

    projection = line_segs[:,:2] + coords1d[:,None]*dir_vec

    dist_to_line = torch.sum((projection-points)**2,dim=-1)
...
def get_segment_overlap(seg_coord1d):
    ...
    overlap = ((seg_coord1d[..., 1] > 0) * (seg_coord1d[..., 0] < 1)
               * (torch.min(seg_coord1d[..., 1], torch.ones_like(seg_coord1d[..., 1]))
                  - torch.max(seg_coord1d[..., 0], torch.zeros_like(seg_coord1d[..., 0]))))
    return overlap
class VolSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        t1, do1 = project_point_to_line(lines2d_gt, lines2d[:,:2])
        t2, do2 = project_point_to_line(lines2d_gt, lines2d[:,2:])
        t = torch.stack((t1,t2),dim=-1).detach()
        is_overlap = get_segment_overlap(t)>0.5
        is_overlap *= torch.max(do1.detach(),do2.detach())<100

        lorth_loss = (torch.sum(do1*is_overlap)+torch.sum(do2*is_overlap))/torch.sum(is_overlap).clamp_min(1)
        # torch.cross(query_2d-lines2d_gt[:,:2],lines2d_gt[:,2:]-lines2d_gt[:,:2])
