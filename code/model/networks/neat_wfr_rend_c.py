    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            if self.dbscan_enabled:
                junctions3d = lines3d.detach().cpu().numpy().reshape(-1,3)
                with torch.no_grad():
                    junctions3d_glb = self.ffn(self.latents).detach().cpu().numpy()

                junctions3d = np.concatenate((junctions3d,junctions3d_glb),axis=0)
                junctions3d = self.cluster_dbscan(junctions3d, eps=0.01,min_samples=2)
