...
class VolSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        if assign[0].shape[0] >0:
            ...
            loss_j3d = torch.tensor(0,device='cuda',dtype=torch.float32)
            loss_j2d = torch.tensor(0,device='cuda',dtype=torch.float32)
            loss_j2d_u = torch.tensor(0,device='cuda',dtype=torch.float32)
