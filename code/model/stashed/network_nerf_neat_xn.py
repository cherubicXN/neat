...
class NeRFNetwork(nn.Module):
    ...
    def project2D(self, K,R,T, points3d):
        ...
        x = K@(R@X.t()+T)
        x = x.t()
        z = torch.where(x[:,-1:]>0,x[:,-1:].clamp_min(1e-6),x[:,-1:].clamp_max(-1e-6))
        # print(x[:,-1].abs().min())
        x = x/z
        x = x.reshape(*shape)[...,:2]
        return x
    ...
    def forward(self, input):
        ...
        lines3d_aux = torch.sum(weights[:,:,None,None]*lines3d,dim=1)
        # import pdb; pdb.set_trace()
        ...
        center_queries = lines3d.mean(dim=2)
        _, query_feats = self.rendering_network(center_queries.detach(), center_queries.detach())
        lines3d_flatten = self.center_network(center_queries.reshape(-1,3),query_feats.reshape(-1,256).detach())
        lines2d_flatten = self.project2D(intrinsics[0,:3,:3], R, T, lines3d_flatten).reshape(-1,4)
        lines3d = lines3d_flatten.reshape(-1,N_samples,2,3)
        lines2d = lines2d_flatten.reshape(-1,N_samples,4)
        import pdb; pdb.set_trace()
        """Learning Attraction Fields: BEGIN"""
        output['lines3d'] = lines3d
        output['lines2d'] = lines2d
        output['lines2d-aux'] = lines2d_aux.reshape(-1,4)
