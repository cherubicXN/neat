...
def run(**kwargs):
    ...
    lines3d = []
    HyNet = HyNet.cuda()
    for i in tqdm(range(len(wireframes)-1)):
    # for i in range(0,1):
        max_p = i+1
        max_v = 0
        lines3d_i = []
        ...
        Rotations = torch.eye(3)[None].repeat(len(wireframes),1,1)
        Translations = torch.zeros(len(wireframes),3,1)
        ...
        # for j in range(i+1,2):
            ...
            idx = torch.stack((idx0[:,-1],idx1[:,-1]),dim=-1).tolist()
            for idi, idj in idx:
                tracks[idi].append((j,idj))

        X3d = torch.zeros((wireframes[i].vertices.shape[0],3))
        mask = torch.zeros(wireframes[i].vertices.shape[0],dtype=torch.bool)
        for track_id in tracks:
            match = tracks[track_id]
            x = [wireframes[i].vertices[track_id]]
            P = [K[i][0,:3,:3]@torch.cat((Rotations[i],Translations[i]),dim=-1)]
            for view_id, match_id in match:
                x.append(wireframes[view_id].vertices[match_id])
                P.append(
                    K[view_id][0,:3,:3]@torch.cat((Rotations[view_id],Translations[view_id]),dim=-1)
                )
            
            x = torch.stack(x,dim=0)
            P = torch.stack(P,dim=0)
            X = []
            xh = kornia.geometry.conversions.convert_points_to_homogeneous(x)
            xhat = kornia.geometry.epipolar.cross_product_matrix(xh)
            # kornia.geometry.epipolar.cross_product_matrix(x)
            X = xhat@P#.reshape(-1,4)
            X = X[:,:2].reshape(-1,4)

            _, _, V = torch.svd(X)

            points3d_h = V[...,-1]
            points3d = kornia.geometry.conversions.convert_points_from_homogeneous(points3d_h[None])[0]

            Xw =  poses[i][0,:3,:3]@points3d + poses[i][0,:3,3]

            if Xw.abs().max()<3:
                X3d[track_id] = Xw
                mask[track_id] = True

        edges_i = wireframes[i].edges 
        edge_mask = mask[edges_i].all(dim=-1)*(wireframes[i].weights>0.5)
        lines = X3d[edges_i[edge_mask]]
        lines3d.append(lines)
    ...
    scores = [verifyByDensity(l, model) for l in lines3d]
    scenes = []
    for line, score in zip(lines3d,scores):
        tm = trimesh.load_path(line[score>0.5].numpy())
        scenes.append(tm)
    
    trimesh.Scene(scenes).show()
