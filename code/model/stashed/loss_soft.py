...
def project_point_to_line(line_segs, points):
    ...
    dir_vec = (line_segs[:, 1] - line_segs[:, 0])
    coords1d = (((points - line_segs[:, 0]) * dir_vec).sum(dim=1)
                / torch.norm(dir_vec, dim=1) ** 2)
    # coords1d is of shape (n_lines, n_points)
    
    # Compute the orthogonal distance of the points to each line
    projection = line_segs[:, 0] + coords1d[:, None] * dir_vec
    dist_to_line = torch.norm(projection - points, dim=-1)

    return projection, dist_to_line
...
class VolSDFLoss(nn.Module):
    ...
    def get_line_loss(self, lines2d, lines2d_gt, lines_weight, threshold = 100):
        ...
        if threshold is not None:
            ...
            labels = torch.where(labels, 
                torch.ones_like(labels,dtype=torch.float32),
                0.01*torch.ones_like(labels,dtype=torch.float32))
        else:
            labels = torch.ones_like(lines2d_loss)
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        lines2d_loss, threshold = self.get_line_loss(lines2d, lines2d_gt, lines_weight,threshold=None)

        # proj_point1, proj_dis1 = project_point_to_line(lines2d_gt.reshape(-1,2,2),lines2d[:,[0,1]])
        # proj_point2, proj_dis2 = project_point_to_line(lines2d_gt.reshape(-1,2,2),lines2d[:,[2,3]])

        # loss_proj1 = torch.abs(proj_point1-lines2d[:,:2]).sum(dim=-1)
        # loss_proj2 = torch.abs(proj_point2-lines2d[:,2:]).sum(dim=-1)
        # loss_proj = torch.mean(loss_proj1+loss_proj2)/2.0
