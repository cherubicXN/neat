...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            sdf_pts = self.implicit_network.get_sdf_vals(input['pts'].cuda()[0])
            grad_pts = self.implicit_network.gradient(input['pts'].cuda()[0])
            

            output['sdf_pts'] = sdf_pts
            output['grad_pts'] = grad_pts
        if self.training:
