...
class CenterNetwork(nn.Module):
    ...
    def forward(self, points, normals, feature_vectors):
        ...
        x = torch.cat([rendering_input,enc],dim=-1)
        # x = rendering_input
...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.center_net = CenterNetwork(
            self.feature_vector_size, 6, 6, [256, 256],
            divide_factor=6,
            base_size=512,
            num_levels=10
        )
    ...
    def forward(self, input):
        ...
        csdf, cfeat, cgrads = self.implicit_network.get_outputs(centers)
        lines3d_aux = self.center_net(centers,cgrads, cfeat)
