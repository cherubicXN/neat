...
def trimesh_points(points):
    pc = trimesh.points.PointCloud(points.detach().cpu())
    colors = torch.randint(0,255,(1,3))
...
class AttentionNetwork(nn.Module):
    def __init__(self, dim_feat, num_heads = 1, num_layers = 4):

        super().__init__()
        self.num_layers = num_layers
        for l in range(0, self.num_layers):
            lin = nn.Linear(dim_feat,dim_feat)
            # torch.nn.init.constant_(lin.bias, 0.0)
            # torch.nn.init.normal_(lin.weight, 0.0, np.sqrt(2)/np.sqrt(dim_feat))
...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.embedding_cfg = conf.get_config('embedding')

        emb = torch.randn((self.embedding_cfg.get_int('num_queries'),256))
        # emb = emb/ emb.norm(dim=-1,keepdim=True)
        self.emb = nn.Parameter(emb,requires_grad=True)
        self.multi_head_attn = nn.ModuleList(
            [
                nn.MultiheadAttention(256,4),
                # nn.MultiheadAttention(256,4),
            ]
            )
        # self.mlp = AttentionNetwork(256,num_heads=1,num_layers=4)
        ...
        # torch.nn.init.normal_(self.head.weight, 0.0, 0.00001)
        # nn.utils.weight_norm(self.head)
    ...
    def forward(self, input):
        ...
        points2d = uv.clone()[0]
        lines2d_gt = input['wireframe'][0].line_segments(0.05).cuda()
        points3d_sdf, points_features, points_gradients = self.implicit_network.get_outputs(points3d)
        
        attn_feat, attn_map = self.multi_head_attn[0](points_features, self.emb, self.emb)
        points_features = points_features + self.mlp(attn_feat)
        # points_features = self.mlp(points_features)
        ...
        # label_set = attn_map.detach().max(dim=-1)[1].unique()
        # assignment = attn_map.detach().max(dim=-1)[1]
        # tm_all = trimesh.load_path(lines3d.detach().cpu())
        # for l in label_set:
        #     temp = lines3d[assignment==l].detach()
        #     centers = 0.5*(temp[:,0]+temp[:,1])
        #     tmp = trimesh_points(centers)
        #     trimesh.Scene([tm_all,tmp]).show()
            # tm_i = trimesh.load_path(lines3d[assignment==l].detach().cpu())
            # colors = torch.tensor([[255,0,0]]).repeat(len(tm_i.entities),1)
            # tm_i.colors = colors
            # trimesh.Scene([tm_i,tm_all]).show()
            # trimesh.Scene([tm_all,tm_i]).show()
            # import pdb; pdb.set_trace()
        ...
        if not self.training:
            output['attn_map'] = attn_map.detach()
        # output['lines3d-query'] = query_lines3d
        # output['lines2d-query'] = query_lines2d
