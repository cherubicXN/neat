...
class AttractionFieldNetwork(nn.Module):
    ...
    def forward(self, points, feature_vectors):
        ...
        x = torch.cat((points,feature_vectors),dim=-1)
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        lines3d_flt = self.attraction_network(points_flat,feature_vectors.detach())
        lines3d = lines3d_flt.reshape(-1,N_samples,2,3)
        lines3d = torch.sum(lines3d*weights[:,:,None,None].detach(),dim=1)
        ...
        points3d_sdf, points_features, points_gradients = self.implicit_network.get_outputs(points3d.detach())
        ...
        if not self.training:
            ...
            lines3d  = self.attraction_network.forward(l3d.detach(),  points_features.detach())
        # lines3d = torch.stack((e1,e2),dim=1)
            lines2d = self.project2D(intrinsics[0,:3,:3], R, T, lines3d)
