...
class AttractionFieldNetwork(nn.Module):
    def __init__(self,
        ...
        self.feature = nn.Sequential(
            nn.Linear(3, feature_vector_size),
            nn.ReLU(),
            nn.Linear(feature_vector_size,num_instances),
            nn.Sigmoid()
            )
    ...
    def forward(self, points, normals, feature_vectors):
        ...
        lines_code = self.feature(lines.mean(dim=1).detach())
        # lines_ffn = self.feature_ffn(lines_code_logits).reshape(-1,2,3)
        # lines_code = lines_code_logits.sigmoid()
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        output = {
            ...
            'unilines': input['lines_uniq'][0].cuda(),
        ...
        # lines2d_aux = self.project2D(intrinsics[0,:3,:3], R, T, lines3d_ffn)
