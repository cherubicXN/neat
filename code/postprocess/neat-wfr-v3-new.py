...
def junction_retrieval(junctions3d, eval_dataloader, model, *, 
                        mindis_th = 25, min_visible_views=1, device='cuda'):
    
    visibility = torch.zeros((junctions3d.shape[0],),device=device)
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        juncs2d_gt = model_input['wireframe'][0].vertices
        juncs2d_all = model.project2D(K,R,T,junctions3d).reshape(-1,2)
        dist = torch.norm((juncs2d_all[:,None]-juncs2d_gt[None].to(device)),dim=-1)

        assign = linear_sum_assignment(dist.cpu().numpy())
        assign = [torch.tensor(a,device=device) for a in assign]
        dist_asg = dist[assign[0],assign[1]]
        visibility[assign[0][dist_asg<5]] += 1
    ...
    pcd1 = trimesh.points.PointCloud(junctions3d[visibility>5].cpu())
    pcd2 = trimesh.points.PointCloud(junctions3d[visibility<=5].cpu())
    import pdb; pdb.set_trace()
def visibility_checking(lines3d_all, eval_dataloader, model, *, 
    ...
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        junctions2d, edges2d = wireframe.vertices, wireframe.edges
        wg = torch.zeros((junctions2d.shape[0],junctions2d.shape[0]),device=device)
        wg[edges2d[:,0],edges2d[:,1]] = wireframe.weights.to(device)
        wg[edges2d[:,1],edges2d[:,0]] = wireframe.weights.to(device)

        lines2d_all = model.project2D(K,R,T,lines3d_all).reshape(-1,4)
        dist1 = torch.norm((lines2d_all[:,None,:2]-junctions2d[None].to(device)),dim=-1)
        d1,i1 = dist1.min(dim=1)
        dist2 = torch.norm((lines2d_all[:,None,2:]-junctions2d[None].to(device)),dim=-1)
        d2,i2 = dist2.min(dim=1)
        ...
        lines3d_visibility[cond1*cond2,indices[0]] = True
...
def wireframe_recon(**kwargs):
    ...
    # junction_retrieval(initial_recon_results['junctions3d_initial'], eval_dataloader, model, mindis_th = 100, min_visible_views=5)
    # 5 views for dtu24
    lines3d_wfi_checked = visibility_checking(initial_recon_results['lines3d_wfi'], eval_dataloader, model, mindis_th = 100, min_visible_views=5)
    lines3d_wfr_checked = visibility_checking(initial_recon_results['lines3d_wfr'], eval_dataloader, model, mindis_th = 100, min_visible_views=5)
