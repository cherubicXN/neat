...
class VolSDFTrainRunner():
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            if self.do_vis and epoch % self.plot_freq == 0:
                ...
                plot_data = self.get_plot_data(model_outputs, model_input['pose'], ground_truth['hat'])
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                psnr = rend_util.get_psnr(model_outputs['hat_values'],
                                          ground_truth['hat'].cuda().reshape(-1,4))
                loss_meters.push({'psnr': psnr})
                print(
                    '{0}_{1} [{2}] ({3}/{4}): loss = {5}, rgb_loss = {6} ({9:.3f}), eikonal_loss = {7} ({10:.3f}), psnr = {8} ({11:.3f})'
                        .format(self.expname, self.timestamp, epoch, data_index, self.n_batches, loss.item(),
                                loss_output['rgb_loss'].item(),
                                loss_output['eikonal_loss'].item(),
                                psnr.item(),
                                loss_meters()['rgb_loss'].item(),
                                loss_meters()['eikonal_loss'].item(),
    ...
    def get_plot_data(self, model_outputs, pose, rgb_gt):
        ...
        hat_eval = model_outputs['rgb_values'].reshape(batch_size, num_samples, 4)
