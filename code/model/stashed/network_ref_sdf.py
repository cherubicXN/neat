...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            rays_d_, cam_loc_ = rend_util.get_camera_params(input['pts'].cuda(), pose, intrinsics)
            cam_loc_ = cam_loc_.unsqueeze(1).repeat(1,num_pixels,1).reshape(-1,3)
            rays_d_ = rays_d_.reshape(-1,3)
            _, z_samples_eik_ = self.ray_sampler.get_z_vals(rays_d_, cam_loc_, self)
            z_vals_ = input['z'].reshape(-1,1).cuda()
            # rays_d_ = z_vals_.unsqueeze(2) * rays_d_.unsqueeze(1)
            rays_d_ = z_vals_*rays_d_
            depth_ratio_ = rays_d_.norm(dim=-1)
            points_ = cam_loc_ +rays_d_
            sdf_ = self.implicit_network.get_sdf_vals(points_)
            output['sdf_'] = sdf_
        ...
        if self.training:
            ...
            # eikonal_points = torch.cat([eikonal_points, eik_near_points, points_], 0)
            # eikonal_points = torch.cat([eik_near_points, points_], 0)
            eikonal_points = torch.cat([points_], 0)
