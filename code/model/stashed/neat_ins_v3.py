...
class AttractionFieldNetwork(nn.Module):
    def __init__(self,
        ...
        self.relu = nn.ReLU()
        self.feature = nn.Sequential(nn.Linear(6, feature_vector_size),nn.ReLU(),nn.Linear(feature_vector_size,num_instances),torch.nn.Sigmoid())
    ...
    def forward(self, points, normals, feature_vectors):
        ...
        for l in range(0, self.num_layers - 2):
            ...
            # if l < self.num_layers - 2:
            x = self.relu(x)
        offsets = getattr(self, "lin"+str(self.num_layers-2))(x)
        lines = points[:,None] + offsets.reshape(-1,2,3)
        lines_code = self.feature(lines.reshape(-1,6).detach())
        # import pdb; pdb.set_trace()
        # logits = x[:,6:]
        return lines, lines_code
...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.num_instances = conf.get_int('num_instances')
        ...
        self.attraction_network = AttractionFieldNetwork(self.feature_vector_size,self.num_instances, **conf.get_config('attraction_network'))
    ...
    def forward(self, input):
        ...
        # ins = ins_flat.reshape(-1, N_samples, ins_flat.shape[-1])

        weights = self.volume_rendering(z_vals, sdf)

        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)
        # ins_values = torch.sum(weights.unsqueeze(-1).detach() * ins, 1)
        ...
        output = {
            ...
            # 'ins': ins_values,
            'labels': input['labels'].flatten().cuda(),
            'valid_ins_num': input['labels'].max()+1,
        }
        ...
        lines3d, ins  = self.attraction_network.forward(points3d.detach(), points_gradients.detach(), points_features.detach())
        ...
        if not self.training:
            ...
            # mean_features = points_features.mean(dim=0,keepdim=True)
            # points_features = mean_features.repeat(points3d_sdf.shape[0],1)
            lines3d, ins  = self.attraction_network.forward(l3d.detach(), points_gradients.detach(), points_features.detach())
        # lines3d = torch.stack((e1,e2),dim=1)
            lines2d = self.project2D(intrinsics[0,:3,:3], R, T, lines3d)
        import open3d as o3d
        endpoints = lines3d.reshape(-1,3).detach()
        clusters = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(endpoints.cpu())).cluster_dbscan(0.01,2)
        clusters = torch.tensor(clusters)
        junctions = []
        for lb in clusters.unique():
            if lb == -1:
                continue
            junctions.append(
                endpoints[clusters==lb].mean(dim=0)
            )
        junctions = torch.stack(junctions,dim=0)
        import trimesh
        trimesh.points.PointCloud(junctions.cpu()).show()
        # self.project2D(intrinsics[0,:3,:3], R, T, junctions)
        import pdb; pdb.set_trace()
        output['ins'] = ins
