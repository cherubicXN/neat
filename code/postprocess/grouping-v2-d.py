...
class Description(torch.nn.Module):
    def __init__(self, out_channels = 1024):
        ...
        self.network = torch.nn.Sequential(
            torch.nn.Linear(3,out_channels),
            torch.nn.Sigmoid(),
        )
    def forward(self, x):
...
def wireframe_recon(**kwargs):
    ...
    line_path = kwargs['data'][:-4]+'-group-v2d'
    ...
    ffn = Description(1024).to(device=device)
    lines3d_pi_all = ffn(lines3d_all).detach()
    ...
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        lines3d_sel = lines3d_all[mindis<5]
        lines3d_pi = lines3d_pi_all[mindis<5]
        labels_sel = minidx[mindis<5]
        ...
        lines3d_global_id[mindis<5,indices.item()] = pred_label.cuda()
        visibility[mindis<5] += 1
        visibility_score[mindis<5] += torch.exp(-mindis)[mindis<5]
