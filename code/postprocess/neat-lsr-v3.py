...
def wireframe_recon(**kwargs):
    ...
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        lines = model_input['lines'][0][mask[0]].cuda()
        labels = model_input['labels'][0][mask[0]]
        ...
        #     idx = (label==labels).nonzero().flatten()
        #     uv = model_input['uv'][:,idx]
        #     uv_proj = model_input['uv_proj'][:,idx]
        ...
        for label in tqdm(labels.unique()):
            ...
            dis1 = torch.sum((lines2d_ - lines2d[:,:-1])**2,dim=-1)
            dis2 = torch.sum((lines2d_[:,[2,3,0,1]] - lines2d[:,:-1])**2,dim=-1)
            
            dis = torch.min(dis1,dis2)
            lines3d_[dis1>dis2] = lines3d_[dis1>dis2][:,[1,0]]
            is_correct = dis<10

            if is_correct.sum() == 0:
                continue
            
            # A = torch.ones(points3d_.shape[0],4)
            # A[:,:-1] = points3d_
            # _, lambda_, line_coef = torch.svd(A)
            # line_coef = line_coef[:,-1]
            # lambda_ = lambda_[-1]
            sdf_vals, feats, _ = model.implicit_network.get_outputs(points3d_)
            sdf_vals = sdf_vals.abs().flatten().detach()

            lines3d_by_dict[label.item()].append(lines3d_[is_correct])
            points3d_by_dict[label.item()].append(points3d_[sdf_vals<1e-3].detach())
            # points3d_by_dict[label.item()].append(points3d_)
        lines3d = []
        for key, val in lines3d_by_dict.items():
            val = torch.cat(val)
            points3d_by_dict[key] = torch.cat(points3d_by_dict[key])
            P = val.reshape(-1,3).mean(dim=0)
            L_points = val.reshape(-1,3).t()
            C = (torch.eye(L_points.shape[1]) - 1/L_points.shape[1]*torch.ones(L_points.shape[1],L_points.shape[1]))
            # C = C@torch.ones(L_points.shape[1],L_points.shape[1])
            Scat = L_points@C.cuda()@L_points.t()
            direction = torch.svd(Scat)[-1][:,0]

            t1_ =  torch.sum((val[:,0]-P)*direction,dim=-1)
            t2_ =  torch.sum((val[:,1]-P)*direction,dim=-1)

            t1 = torch.min(t1_,t2_)
            t2 = torch.max(t1_,t2_)
            # temp = torch.stack((P+direction,P-direction))

            if val.shape[0] == 1:
                # lines3d.append(val[0])
                lines_kept = val[0]
            else:
                lines_kept = torch.stack((P+direction*t1.mean(),P-direction*t2.mean()))
