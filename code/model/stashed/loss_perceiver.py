...
class VolSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        juncs2d_gt = model_outputs['wireframe-gt'][0].vertices.cuda()
        juncs2d_pt = model_outputs['juncs2d']

        jdist = torch.sum((juncs2d_pt[:,None] - juncs2d_gt[None])**2,dim=-1).detach().cpu().numpy()
        assignment_p, assignment_g = linear_sum_assignment(jdist)
        
        loss_juncs = torch.sum((juncs2d_pt[assignment_p]-juncs2d_gt[assignment_g]).abs(),dim=-1)
        jmask = loss_juncs.detach()<100
        loss_juncs = torch.mean(loss_juncs*jmask)
