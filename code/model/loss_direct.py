...
class VolSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        # labels = (lines2d_loss.detach()<10).long()
        ...
        # loss_emb =  torch.nn.functional.mse_loss(model_outputs['emb'],model_outputs['emball'].detach())
        
        lines2d_emb = model_outputs['lines2d-emb'].reshape(-1,4)
        lines2d_egt = model_outputs['lines2d-egt'].reshape(-1,5)
        lines2d_egt, weight = lines2d_egt.split(4,dim=-1)
        
        dist1 = torch.sum((lines2d_emb-lines2d_egt)**2,dim=-1,keepdim=True).detach()
        dist2 = torch.sum((lines2d_emb-lines2d_egt[:,[2,3,0,1]])**2,dim=-1,keepdim=True).detach()

        lines_tgt = torch.where(dist1<dist2,lines2d_egt,lines2d_egt[:,[2,3,0,1]])

        emb_loss = torch.abs(lines2d_emb-lines_tgt).mean(dim=-1)
        emb_loss = torch.mean(emb_loss*weight.flatten())
