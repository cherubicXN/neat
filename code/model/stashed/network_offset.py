...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        for it in range(2):
            if it == 0:
                x3d = torch.sum(lines3d,dim=1)*0.5
            else:
                x3d = torch.sum(lines3d_aux[-1],dim=1)*0.5
            _, x3d_feat, x3d_grad = self.implicit_network.get_outputs(x3d.detach())
            x3d_line = self.attraction_network.forward(x3d.detach(), x3d_grad.detach(), x3d_feat.detach())
            x2d_line = self.project2D(intrinsics[0,:3,:3], R, T, x3d_line)
            lines3d_aux.append(x3d_line)
            lines2d_aux.append(x2d_line)
        ...
        #         lines2d_sel = lines2d[idx_sel].reshape(-1,4).detach()
        #         lines2d_sel_gt = input['lines_uniq'][0,lb].cuda()
        #         with torch.no_grad():
        #             dis1 = torch.sum((lines2d_sel-lines2d_sel_gt[None,:-1])**2,dim=-1)
        #             dis2 = torch.sum((lines2d_sel-lines2d_sel_gt[None,:-1][:,[2,3,0,1]])**2,dim=-1)
        #             dis_ = torch.min(dis1,dis2)
        #         is_valid = dis_<100
        #         if is_valid.sum() == 0:
        #             continue
