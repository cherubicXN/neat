...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        for lines in tqdm(self.lines):
            mask, md_angles, labels, att_points = self.compute_point_line_attraction(lines)
            md_angles = md_angles[:,None].repeat((1,3))
            self.masks.append(mask)
            self.md_angles.append(md_angles)
    ...
    def compute_point_line_attraction(self, lines):
        ...
        md_angle = (md_angle + np.pi)/(2*np.pi)
        ...
        return mask.cpu().reshape(-1), md_angle.cpu().reshape(-1), labels.cpu().reshape(-1), proj_points.reshape(-1,2)
    ...
    def __getitem__(self, idx):
        ...
        if self.sampling_idx is not None:
            ...
            ground_truth['md'] = self.md_angles[idx][sampling_idx, :]
