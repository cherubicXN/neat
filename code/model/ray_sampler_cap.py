...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    # Invert CDF
    u = u.contiguous().to(weights.device)
...
class UniformSampler(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, N_important=0, take_sphere_intersection=False, far=-1):
    ...
    def get_z_vals_fine(self, z_vals, weights, model):
        ...
        z_samples = sample_pdf(z_vals_mid, weights[...,1:-1], self.N_important, det=model.training, pytest=False)
...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model):
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            with torch.no_grad():
                samples_sdf = model.implicit_network.get_sdf_vals(points_flat,model.ffn(model.latents))
