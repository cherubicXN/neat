...
def wireframe_recon(**kwargs):
    ...
    line_path = os.path.join(wireframe_dir,'{}-d={}-s={}-v4.npz'.format(kwargs['checkpoint'],kwargs['distance'],kwargs['score']))
    ...
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        # for s, lb, lines_gt in zip(split,split_label,split_lines):
            ...
            if 'l3d' in out:
                ...
                points3d_ = torch.zeros_like(lines3d_[:,0])
            ...
            for label_ in label_set:
                ...
                points3d_kpt = points3d_valid[idx]
                sdf_vals = model.implicit_network.get_sdf_vals(points3d_kpt).detach().flatten()
                points3d_by_dict[label_.item()].append(points3d_kpt[sdf_vals<1e-3])
               
        
        for key, val in lines3d_by_dict.items():
            val = torch.cat(val)
            points3d_by_dict[key] = torch.cat(points3d_by_dict[key])
            if val.shape[0] < 2 or points3d_by_dict[key].shape[0] < 2:
                continue
            # l3d_feat = model.implicit_network(points3d_by_dict[key])[:,1:]
            # latents, latent_attn = model.attention(model.latents,l3d_feat,l3d_feat)
            # lines3d_query = model.ffn(latents).reshape(-1,3)

            # import pdb; pdb.set_trace()

            support_pts = points3d_by_dict[key]

            support_dis = torch.cross(support_pts[:,None]-val[None,:,1],support_pts[:,None]-val[None,:,0]).norm(dim=-1)/torch.norm(val[:,1]-val[:,0],dim=-1).reshape(1,-1)
            # print(support_dis.shape)
            # mindis, minidx = support_dis.min(dim=0)
            support_dis = support_dis.mean(dim=0)

            if val.shape[0] == 1:
                # lines3d.append(val[0])
                lines_kept = val[0]
            else:
                weight = torch.softmax(-support_dis*1000,dim=0)
                # lines_kept = val.mean(dim=0)
                lines_kept = torch.sum(weight[:,None,None]*val,dim=0)
