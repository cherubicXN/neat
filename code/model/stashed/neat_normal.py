...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        lines3d = torch.sum(weights.detach()[:,:,None,None]*lines3d_v, dim=1)
        ...
        if self.training:
            ...
            grad_alpha = self.implicit_network.gradient(points3d)
            output['grad_alpha'] = grad_alpha/grad_alpha.norm(2,-1,keepdim=True)
            offset = lines3d-points3d[:,None]
            normal2 = torch.cross(offset[:,0],offset[:,1])
            normal2 = normal2/normal2.norm(2,-1,keepdim=True)
            output['grad_cross'] = normal2
