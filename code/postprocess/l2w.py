...
def wireframe_recon(**kwargs):
    ...
    connectivity = torch.zeros(lines3d_all.shape[0],lines3d_all.shape[0],device=device)
    for indices, model_input, ground_truth in tqdm(eval_dataloader):    
        ...
        junctions2d = model_input['wireframe'][0].vertices
        weights = model_input['wireframe'][0].weights
        edges = model_input['wireframe'][0].edges
        graph = torch.zeros((junctions2d.shape[0],junctions2d.shape[0]))
        graph[edges[:,0],edges[:,1]] = weights
        graph[edges[:,1],edges[:,0]] = weights

        lines2d_sel = lines2d_all[is_visib]
        
        jid_1 = torch.norm(lines2d_sel[:,None,:2] - junctions2d[None].cuda(),p=2,dim=-1).min(dim=1)[1]
        jid_2 = torch.norm(lines2d_sel[:,None,2:] - junctions2d[None].cuda(),p=2,dim=-1).min(dim=1)[1]
        jid_min = torch.min(jid_1,jid_2)
        jid_max = torch.max(jid_1,jid_2)
        matched_jid = torch.cat((jid_1,jid_2)).unique()

        for jid in matched_jid:
            # jid_min == jid
            is_ok = ((jid_min == jid) + (jid_max==jid))
            edge_idx = is_visib.nonzero()[is_ok].flatten()
            xx, yy = torch.meshgrid(edge_idx,edge_idx)
            xx = xx.flatten()
            yy = yy.flatten()
            connectivity[xx,yy] = 1
