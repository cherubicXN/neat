...
class ImplicitNetwork(nn.Module):
    def __init__(
        ...
        self.line_head = nn.Linear(feature_vector_size, 6)
    ...
    def get_outputs(self, x):
        ...
        gradients = torch.autograd.grad(
            ...
            retain_graph=True,
            only_inputs=True)[0]
        attraction_vectors = self.line_head(feature_vectors.detach())
        lines3d = attraction_vectors.reshape(-1,2,3) + x[:,None].detach()
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        lines3d = torch.sum(lines3d_flt.reshape(-1, N_samples, 2, 3)*weights.detach()[:,:,None,None],dim=1)
        ...
        points3d_sdf, _, points_features, points_gradients = self.implicit_network.get_outputs(points3d)
        ...
        if not self.training:
            points3d_sdf, lines3d, points_features, points_gradients = self.implicit_network.get_outputs(l3d)
