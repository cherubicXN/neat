...
def WireframeVisualizer(lines, render_dir = None, cam_dir = None, rx=0, ry=0,rz=0,t=0, points3d_all = None, show_endpoints = False):
    ...
    for i,key in enumerate(tqdm(keys)):
        ...
        if show_endpoints:
            plt.scatter(lines2d[:,0,0],lines2d[:,0,1],color='b',s=0.4,edgecolors='none',zorder=5)
            plt.scatter(lines2d[:,1,0],lines2d[:,1,1],color='b',s=0.4,edgecolors='none',zorder=5)
...
if __name__ == "__main__":
    ...
    if opt.save:
        if opt.threshold is not None:
            opt.save = opt.data.rstrip('.npz')+'_junctions_record_{}'.format(opt.threshold)
        else:
            opt.save = opt.data.rstrip('.npz')+'_junctions_record'
    else:
        opt.save = None
    ...
    print(lines3d.shape)
    WireframeVisualizer(lines3d,opt.save, None, rx=rx,ry=ry,rz=rz,t=t, points3d_all=points3d, show_endpoints=True)
