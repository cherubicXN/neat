...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.sampling_idx is not None:
            ...
            # randperm = torch.randperm(self.lines3d.shape[0])[:len(self.sampling_idx)]
            # lines_sel = self.lines3d[randperm]
            ...
            # lines2d_sel = lines_p[minidx[is_avail]]
            # import pdb; pdb.set_trace()
            t = torch.linspace(0,1,32).reshape(1,-1,1)
            pts = lines3d_sel[:,:1]*t + lines3d_sel[:,1:]*(1-t)

            pts = pts.reshape(-1,3)
            randperm = torch.randperm(pts.shape[0])#[:2048]
            # sample['pts'] = pts.reshape(-1,3)
            sample['pts'] = pts[randperm]
