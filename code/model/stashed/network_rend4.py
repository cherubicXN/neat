...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        # sdf = self.implicit_network.get_sdf_vals(lines3d[:,0]*0.5+lines3d[:,1]*0.5)
        ...
        random_t = torch.rand(lines3d.shape[0],device=lines3d.device).reshape(-1,1)
        if not self.training:
            random_t = random_t*0+0.5
        points_online = lines3d[:,0]*random_t + lines3d[:,1]*(1-random_t)
        sdf_points, feature_points, grad_points = self.implicit_network.get_outputs(points_online)

        lines3d_aux = self.attraction_network(points_online.detach(), grad_points.detach(), feature_points.detach())
