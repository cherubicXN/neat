...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        # line_ray_u, _ = rend_util.get_camera_params(input['lines'][:,:,:2], pose, intrinsics)
        # line_ray_v, _ = rend_util.get_camera_params(input['lines'][:,:,2:4], pose, intrinsics)
        line_ray_d = line_ray_d.reshape(-1,3)
        # line_ray_u = line_ray_u.reshape(-1,3)
        # line_ray_v = line_ray_v.reshape(-1,3)
        line_ray_o = line_ray_o.repeat(line_ray_d.shape[0],1)

        t = torch.sum((points3d-line_ray_o)*points_gradients,dim=-1)/torch.sum(line_ray_d*points_gradients,dim=-1)
        # t1 = torch.sum((points3d-line_ray_o)*points_gradients,dim=-1)/torch.sum(line_ray_u*points_gradients,dim=-1)
        # t2 = torch.sum((points3d-line_ray_o)*points_gradients,dim=-1)/torch.sum(line_ray_v*points_gradients,dim=-1)

        # e1 = line_ray_o + line_ray_u*t1.unsqueeze(-1)
        # e2 = line_ray_o + line_ray_v*t2.unsqueeze(-1)
