...
class AttractionFieldNetwork(nn.Module):
    def __init__(self,
        ...
        self.embed_fn = get_embedder(6,6)
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l < self.num_layers - 2:
                x = self.relu(x)
        radiance = self.sigmoid(x[:,:3])
        offset = x[:,3:].reshape(-1,2,3)
        lines3d = points[:,None] + offset
        return radiance, lines3d
...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.attraction_network = AttractionFieldNetwork(294, **conf.get_config('attraction_network'))
    ...
    def forward(self, input):
        ...
        rgb_flat, lines_flat = self.rendering_network(points_flat, gradients, dirs_flat, feature_vectors)
        rgb = rgb_flat.reshape(-1, N_samples, 3)
        lines3d_v = lines_flat.reshape(-1, N_samples, 2, 3)
        ...
        lines3d_aux = torch.sum(weights.detach()[:,:,None,None]*lines3d_v, dim=1)
        ...
        lines3d = self.attraction_network.forward(points3d, points_gradients, lines3d_v.mean(dim=2).reshape(-1,N_samples*3))
        ...
        if not self.training:
            ...
            # offset = lines3d-points3d[:,None]
            # normal2 = torch.cross(offset[:,0],offset[:,1])
            # normal2 = normal2/normal2.norm(2,-1,keepdim=True)
            # output['normal_att'] = normal2
        return output
