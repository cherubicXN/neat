...
class AttractionFieldNetwork(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        dims = [d_in + feature_vector_size] + dims + [d_out+1]
        self.num_layers = len(dims)
        for l in range(0, self.num_layers - 1):
            out_dim = dims[l+1]
            lin = nn.Linear(dims[l], out_dim)
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        line_ray_d = line_ray_d.reshape(points.shape[0],1,3).repeat(1,points.shape[1],1)
        line_ray_o = line_ray_o.reshape(1,1,3).repeat(points.shape[0],points.shape[1],1)
        gradients = gradients.reshape(points.shape)
        t = torch.sum((points-line_ray_o)*gradients,dim=-1)
        td = torch.sum(line_ray_d*gradients,dim=-1)
        # td = torch.where(td>0,td.clamp_min(1e-10),-td.clamp_max(1e-10))
        t = t/td
        t = t.clamp(min=self.ray_sampler.near,max=self.ray_sampler.far)
        t = t.detach()
        t = t.sort(dim=-1)[0]

        points_ol = line_ray_o + t.unsqueeze(-1)*line_ray_d
        points_ol_flt = points_ol.reshape(-1,3)
        sdf_ol, feats_ol, grads_ol = self.implicit_network.get_outputs(points_ol_flt)

        lines3d_smp, sigma_smp = self.attraction_network.forward(points_ol_flt, grads_ol, feats_ol)
        weights_line = self.volume_rendering_nerf(t, sigma_smp.reshape(*t.shape), t)
        lines3d_smp = lines3d_smp.reshape(*t.shape,2,3)
        lines2d_gt = input['wireframe'][0].line_segments().cuda()
        
        lines3d = torch.sum(lines3d_smp*weights_line[:,:,None,None],dim=1)
        l3d =  torch.sum((line_ray_o + line_ray_d*t.unsqueeze(-1))*weights_line.unsqueeze(-1),dim=1)
        ...
        if self.training:
            ...
            eikonal_points = torch.cat([eikonal_points, eik_near_points,l3d], 0)
    ...
    def volume_rendering_nerf(self, z_vals, sigma, depth_ratio):
        ...
        alpha = 1.-torch.exp(-torch.nn.functional.relu(sigma+noise)*dists) # [N_rays, N_samples]
