...
class VolSDFTrainRunner():
    def __init__(self,**kwargs):
        ...
        if torch.cuda.is_available():
            self.model.cuda()
        pretrain_state_dict = torch.load(kwargs['pretrain'])
        pretrained_dict = {}
        for key, val in pretrain_state_dict['model_state_dict'].items():
            # if 'rendering' in key:
            if 'implicit' in key:
                pretrained_dict[key] = val
        # self.model.load_state_dict(pretrain_state_dict['model_state_dict'],strict=False)
    ...
    def valid(self):
        ...
        for indices, model_input, ground_truth in tqdm(self.plot_dataloader):
            ...
            split = utils.split_input(model_input, mask.sum().item(), n_pixels=self.split_n_pixels)
            split_label = torch.split(labels[mask[0]],self.split_n_pixels)
            split_lines = torch.split(lines[mask[0]],self.split_n_pixels)
            # split_lines = 
            lines3d = []
            ...
            for s, lb, lines_gt in zip(tqdm(split),split_label,split_lines):
                ...
                mask_ = out['lines3d-score'].detach().abs()<5e-3
                if mask_.sum() == 0:
                    continue
            ...
            for key, val in lines3d_by_dict.items():
                ...
                if val.shape[0] == 1:
                    ...
                    continue
                # trimesh.load_path(val).show()
                rand_line = val[np.random.choice(val.shape[0],1)]
                line_len = torch.norm(rand_line[0,1]-rand_line[0,0],2,dim=-1)

                dis = torch.sum((rand_line-val)**2,dim=-1).mean(dim=-1)
                is_valid = dis<line_len*1e-3
                if is_valid.sum() == 0:
                    continue
                lines_kept = val[is_valid].mean(dim=0)
                lines3d.append(lines_kept)
            lines3d = torch.stack(lines3d,dim=0).cpu()
            lines3d_all.append(trimesh.load_path(lines3d))
            if len(lines3d_all)%10 == 0:
                trimesh.Scene(lines3d_all).show()
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                # print(
                self.logger.info(
                    '{0}_{1} [{2}] ({3}/{4}): {5}, psnr = {6} ({7:.3f}), lr = {8}'
