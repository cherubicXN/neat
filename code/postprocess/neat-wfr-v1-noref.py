...
def wireframe_recon(**kwargs):
    ...
    line_path = os.path.join(wireframe_dir,'{}-wfr-noref.npz'.format(kwargs['checkpoint']))
    ...
    is_activated = torch.tensor([k for k in gjc_dict.keys()])
    junctions3d_refined = global_junctions[is_activated]
