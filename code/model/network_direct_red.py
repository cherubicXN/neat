...
class AttractionFieldNetwork(nn.Module):
    def __init__(self,
        ...
        dims_emb = [d_in + feature_vector_size] + dims_emb #+ [d_out]
        dims_out = [dims_emb[-1]] + dims_pred + [d_out]
        self.num_emb_layers = len(dims_emb)
        self.num_out_layers = len(dims_out)
        for l in range(0, self.num_emb_layers - 1):
            out_dim = dims_emb[l+1]
            lin = nn.Linear(dims_emb[l], out_dim)
            if weight_norm:
                lin = nn.utils.weight_norm(lin)
            setattr(self, "emb"+str(l), lin)

        for l in range(0, self.num_out_layers - 1):
            out_dim = dims_out[l+1]
            lin = nn.Linear(dims_out[l], out_dim)
            # torch.nn.init.constant_(lin.bias, 0.0)
            # torch.nn.init.constant_(lin.weight, 0.000001)
    ...
    def forward(self, points, normals, feature_vectors):
        ...
        for l in range(1, self.num_emb_layers - 1):
            lin = getattr(self, "emb" + str(l))

            x = lin(x)

            if l < self.num_emb_layers - 2:
                x = self.relu(x)
        # x = self.sigmoid(x)
        emb = x
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        lines3d_flt, embedding_pts = self.attraction_network.forward(points_flat,gradients,feature_vectors)
        lines3d = lines3d_flt.reshape(-1,N_samples,6)
        embedding_pts = embedding_pts.reshape(-1,N_samples,embedding_pts.shape[-1])

        lines3d = torch.sum(weights.unsqueeze(-1)*lines3d,dim=1)
        lines3d = lines3d.reshape(-1,2,3)
        lines_emb = torch.sum(weights.unsqueeze(-1)*embedding_pts,dim=1)
        ...
        if self.training:
            region_labels = input['labels'].flatten()
            label_set = region_labels.flatten().unique().sort()[0]
            emb_mean = []
            emb_gt = []
            for lb in label_set:
                idx_sel = (region_labels==lb).nonzero().flatten()
                emb_i = lines_emb[idx_sel].mean(dim=0)
                emb_mean.append(emb_i)
                emb_gt.append(input['lines_uniq'][0,lb])

            emb_mean = torch.stack(emb_mean,dim=0)
            emb_gt = torch.stack(emb_gt,dim=0)

            lines3d_em = self.attraction_network.forward_from_emb(emb_mean).reshape(-1,2,3)
            lines2d_em = self.project2D(intrinsics[0,:3,:3], R, T, lines3d_em)

            output['lines3d-emb'] = lines3d_em
            output['lines2d-emb'] = lines2d_em
            output['lines2d-egt'] = emb_gt.cuda()

        output['emb'] = lines_emb
