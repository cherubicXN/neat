...
class ImplicitNetwork(nn.Module):
    def __init__(
        ...
        line_pred = nn.Linear(feature_vector_size,6)
        if weight_norm:
            line_pred = nn.utils.weight_norm(line_pred)    
    ...
    def get_outputs(self, x):
        ...
        lines_offset = self.line_pred(self.softplus(feature_vectors)).reshape(-1,2,3)
        lines3d = lines_offset.reshape(-1,2,3) + x[:,None]
        return sdf, lines3d, feature_vectors, gradients

    def get_sdf_vals(self, x):
        # sdf = self.forward(x)[:,:1]
        out = self.forward(x)
        sdf, feature_vectors = out[:,:1], out[:,1:]
        # lines_offset = self.line_pred(self.softplus(feature_vectors)).reshape(-1,2,3)
        # lines = self.line_pred(self.softplus(feature_vectors)).reshape(-1,2,3)
        # lines_dir = lines[:,1] - lines[:,0]
        # lines_dis = torch.cross(x-lines[:,0],lines_dir).norm(dim=-1)/(torch.norm(lines_dir,dim=-1)+1e-10)
        # lines_dis = lines_dis.clamp(max=1.0)[:,None]
        # lines_dis = torch.sign(sdf)*lines_dis
        # sdf = torch.minimum(sdf,lines_dis)
        ''' Clamping the SDF with the scene bounding sphere, so that all rays are eventually occluded '''
...
class VolSDFNetwork(nn.Module):
    ...
    def forward(self, input):
        ...
        sdf, lines3d_flt, feature_vectors, gradients = self.implicit_network.get_outputs(points_flat)
        rgb_flat = self.rendering_network(points_flat, gradients, dirs_flat, feature_vectors)
        rgb = rgb_flat.reshape(-1, N_samples, 3)

        lines_dir = lines3d_flt[:,1] - lines3d_flt[:,0]
        lines_dis = torch.cross(points_flat-lines3d_flt[:,0],lines_dir).norm(dim=-1)/(torch.norm(lines_dir,dim=-1)+1e-10)
        lines_dis = lines_dis.clamp(max=1.0)

        lines_dis = lines_dis.reshape(-1,N_samples)
        # weights = self.volume_rendering(z_vals, -lines_dis)
        weights = self.volume_rendering(z_vals, sdf)
        weights = weights*(1-lines_dis)
        

        # weights = torch.sqrt(weights*weights_line)
        # import pdb; pdb.set_trace()
        weights = weights/(torch.sum(weights,dim=-1,keepdim=True)+1e-10)
        # weights = (1-lines_dis)*weights

        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)

        lines3d = lines3d_flt.reshape(-1, N_samples, 2,3)
