...
class CenterNetwork(nn.Module):
    def __init__(
        ...
        self.encoding = HashEncoder(input_dim=3, num_levels=num_levels, level_dim=level_dim, per_level_scale=2, base_resolution=base_size,
    ...
    def forward(self, points):
        ...
        enc = self.encoding(points/self.divide_factor)
        x = torch.cat([points,enc],dim=-1)
        # x = rendering_input
...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.attraction_network = CenterNetwork(self.feature_vector_size, **conf.get_config('attraction_network'))
        self.center_net = CenterNetwork(
            self.feature_vector_size, **conf.get_config('center_network')
        )
    ...
    def forward(self, input):
        ...
        # lines3d = self.center_net(points_flat, gradients, feature_vectors)
        ...
        lines2d_aux = []
        for i in range(1):
            lines3d_aux_ = self.center_net(centers)#,cgrads, cfeat)
            lines2d_aux_ = self.project2D(intrinsics[0,:3,:3], R, T, lines3d_aux_)
            lines3d_aux.append(lines3d_aux_)
            lines2d_aux.append(lines2d_aux_)
            centers = torch.sum(lines3d_aux_,dim=1)*0.5
        output['lines3d-aux'] = lines3d_aux
        output['lines2d-aux'] = lines2d_aux

        if not self.training:
            output['lines3d'] = lines3d_aux[-1]
            output['lines2d'] = lines2d_aux[-1]
