...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        emb = torch.randn(128,3)
        emb = emb/emb.norm(dim=-1,keepdim=True)

        self.latents = nn.Parameter(emb)
        ...
        # self.perceiver_out = PerceiverSelfOutput(None, 256, 3)
    ...
    def forward(self, input):
        ...
        juncs3d_features = self.implicit_network(juncs3d)[:,1:]
        query_sdf, query_features, query_gradients = self.implicit_network.get_outputs(self.latents)
        query_features,query_attn = self.perceiver[0](query_features,juncs3d_features,juncs3d_features)
        query_features = self.mlp(query_features) + query_features
        juncs3d_pred = self.head(query_features)
        # juncs3d = lines3d.reshape(-1,3)
        # juncs3d_features = self.implicit_network(juncs3d)[:,1:]

        # juncs3d_features = self.perceiver(self.latents[None], inputs=juncs3d_features[None])[0]

        # juncs3d_pred = self.perceiver_out(juncs3d_features)[0]
        juncs2d_pred = self.project2D(intrinsics[0,:3,:3], R, T, juncs3d_pred)
        ...
        output['juncs3d'] = juncs3d_pred
        output['juncs2d'] = juncs2d_pred
