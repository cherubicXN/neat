...
class VolSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        loss_j3d = torch.sum((j3d_local[assign[0]]-j3d_global[assign[1]]).abs(),dim=-1).mean()
        loss_j2d = torch.sum((j2d_local_calib[assign[0]]-j2d_global_calib[assign[1]]).abs(),dim=-1).mean()
