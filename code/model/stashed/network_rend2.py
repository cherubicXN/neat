...
class ImplicitNetwork(nn.Module):
    ...
    def get_outputs(self, x):
        ...
        y = self.forward(x)
        offset = y[:,:6].reshape(-1,2,3)
        features = y[:,6:]
        return x[:,None]+offset, features
...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        # self.attraction_network = ImplicitNetwork(self.feature_vector_size, 0.0 if self.white_bkgd else self.scene_bounding_sphere, **conf.get_config('implicit_network'))
    ...
    def forward(self, input):
        ...
        lines3d_flt, feature_vectors = self.implicit_network.get_outputs(points_flat)

        lines_dis = self.implicit_network.get_sdf_vals(points_flat)
        ...
        rgb_flat = self.rendering_network(points_flat, lines3d_flt[:,1]-lines3d_flt[:,0], dirs_flat, feature_vectors)
        rgb = rgb_flat.reshape(-1, N_samples, 3)

        weights = self.volume_rendering(z_vals, lines_dis)
        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)

        # lines3d = self.attraction_network(points_flat, gradients, dirs_flat, feature_vectors)
