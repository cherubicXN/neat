...
class VolSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        # self.attraction_network2 = AttractionFieldNetwork(self.feature_vector_size, **conf.get_config('attraction_network'))
        ...
        emb = torch.randn((self.embedding_cfg.get_int('num_queries'),3))
    ...
    def forward(self, input):
        ...
        # center3d_sdf, center_features, center_gradients = self.implicit_network.get_outputs(centers3d)
        # lines3d_2 = self.attraction_network.forward(center3d, center_gradients, center_features)
        ...
        query_sdf, query_features, query_gradients = self.implicit_network.get_outputs(self.emb)

        query_points3d = self.emb# - query_sdf*query_gradients
        query_lines3d = self.attraction_network.forward(query_points3d, query_gradients, query_features)
        # query_sdf, query_features, query_gradients = self.implicit_network.get_outputs(query_points3d)
        query_features,query_attn = self.multi_head_attn[0](query_features,points_features,points_features)
        
        query_features = self.mlp(query_features) + query_features
        query_lines3d = self.attraction_network.forward(query_points3d, query_gradients, query_features)
        query_lines2d = self.project2D(intrinsics[0,:3,:3],R,T,query_lines3d)
        # import pdb; pdb.set_trace()
        # import trimesh
