...
def wireframe_recon(
    ...
    line_path = kwargs['data'][:-4]+f"-group-v3-{kwargs['reproj_th']}"
    ...
    lines3d_global_id = lines3d_pi_all.max(dim=1)[1]
    ...
    # lines3d_global_id[visibility<1] = -1
    for line3d, line3d_id, score in zip(lines3d_all, lines3d_global_id,visibility_score):
        if (line3d_id==-1):
            continue

        lines3d_dict[line3d_id.item()].append(line3d.cpu())
        score_dict[line3d_id.item()].append(score.item())
    lines3d_final = []
    import open3d as o3d

    endpoints = lines3d_all.reshape(-1,3)
    clusters = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(endpoints.cpu().numpy())).cluster_dbscan(0.01,2)
    clusters = torch.tensor(clusters)
    junctions3d = []
    for lb in torch.unique(clusters):
        if lb == -1:
            continue
        junctions3d.append(endpoints[clusters==lb].mean(dim=0))
    junctions3d = torch.stack(junctions3d).cpu()
    ffn = model.attraction_network.feature
        # d_output = torch.ones_like()
    graph = torch.zeros((junctions3d.shape[0],junctions3d.shape[0]))
    for key in lines3d_dict.keys():
        lines3d_dict[key] = torch.stack(lines3d_dict[key])
        lines3d_cur = lines3d_dict[key]
        
        dist1 = torch.norm(lines3d_cur[None,:,0] - junctions3d[:,None],p=2,dim=-1)
        dist2 = torch.norm(lines3d_cur[None,:,1] - junctions3d[:,None],p=2,dim=-1)
        
        mdist1, midx1 = dist1.min(dim=0)
        mdist2, midx2 = dist2.min(dim=0)
        mdist = torch.max(mdist1,mdist2)
        length = torch.norm(lines3d_cur[:,0]-lines3d_cur[:,1],dim=-1,p=2)
        is_valid = mdist<0.05*length

        midxi, midxa = torch.min(midx1,midx2), torch.max(midx1,midx2)
        idx_pair = torch.stack([midxi,midxa],dim=-1)
        idx_pair_uni, idx_counts = torch.unique(idx_pair,dim=0,return_counts=True)
        graph[idx_pair_uni[:,0],idx_pair_uni[:,1]] += idx_counts.float()
        graph[idx_pair_uni[:,1],idx_pair_uni[:,0]] += idx_counts.float()


    lines3d_final = junctions3d[(graph.triu()>0).nonzero()]
    tspan = torch.linspace(0, 1,32).reshape(1,-1,1)
    lines3d_pts = lines3d_final[:,:1]*tspan + (lines3d_final[:,1:]*(1-tspan))
    lines3d_pts_sdf, lines3d_pts_features, lines3d_pts_gradients = model.implicit_network.get_outputs(lines3d_pts.reshape(-1,3).cuda())
    lines3d, _ = model.attraction_network.forward(lines3d_pts.reshape(-1,3).cuda(), lines3d_pts_gradients.detach(), lines3d_pts_features.detach())
    lines3d = lines3d.reshape(-1,32,2,3,).detach().cpu()
    # lines3d_points = lines3d_final[:,None,:].repeat(1,32,1) + t[:,None,None]*(lines3d_final[:,None,:].repeat(1,32,1) - lines3d_final[:,None,:].repeat(1,32,1).permute(1,0,2))
    lines3d_scores = torch.zeros(lines3d.shape[0])
    for i in range(lines3d.shape[0]):
        line_cur = lines3d[i]
        line_end = lines3d_final[i]
        distance = get_sAP_line_distance(line_cur,line_end[None]).flatten()
        length = torch.norm(line_cur[:,0]-line_cur[:,1],dim=-1,p=2)
        score_ = (distance<length*0.1).sum()/32
        lines3d_scores[i] = score_
    lines3d_final = lines3d_final[lines3d_scores>0.0]
    # get_sAP_line_distance(lines3d[0],lines3d_final[0][None])
    # import pdb; pdb.set_trace()
