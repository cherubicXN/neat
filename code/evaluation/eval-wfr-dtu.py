...
def evaluate_junctions(junctions3d, global_scale_mat, opt):
    data_pcd = global_scale_mat @ (np.concatenate([junctions3d, np.ones([N, 1])], axis=-1).T) 
    data_pcd = data_pcd[:3].transpose(1, 0)
    data_pcd_o3d = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(data_pcd))
